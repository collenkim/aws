	...
	public void sendEmail(AwsSesV2DTO.ReqSendEmail reqSendEmail) {
		...
		try {
			...
			Destination destination = Destination.builder()
											.toAddresses(reqSendEmail.getReceiveEmailList())
											.ccAddresses(reqSendEmail.getCcEmailList())
											.bccAddresses(reqSendEmail.getBccEmailList())
											.build();//받는사람 세팅

			// Create the subject and body of the message.
			Content subject = Content.builder().charset(Charset.forName("UTF-8").name()).data(reqSendEmail.getEmailTitle()).build();//이메일 제목
			Content textBody = Content.builder().charset(Charset.forName("UTF-8").name()).data(reqSendEmail.getEmailContent()).build();//이메일 내용
			...
			SendEmailRequest request = SendEmailRequest.builder()
													.fromEmailAddress(new InternetAddress(reqSendEmail.getSenderEmail().strip(), reqSendEmail.getSenderNm()).toString())
			...
			String errorMsg = String.format("AWS SES 이메일 발송 시 잘못된 발신자 이메일 주소 및 SenderNm이 확인되어 발송을 하지 않습니다. 요청 senderEmail : %s, senderNm : %s",
			...
			String errorMsg = String.format("AWS SES 발송 시 확인 되지 않은 에러가 발생하여, 해당 이메일은 발송이 되지 않았습니다. errorMsg : %s, 발신 이메일 : %s", e.getMessage(),
					reqSendEmail.getSenderEmail());
			System.out.println(errorMsg);
			throw new SendFailException(errorMsg);
		}
	}
